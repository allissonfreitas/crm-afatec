...
insert into crm.configuracoes (id, nome_empresa, cor_primaria, cor_secundaria)
values (1, 'Afatec', '#0090FE', '#001026')
on conflict (id) do nothing;

-- Origens de lead
insert into crm.origens (nome) values
  ('Ligação recebida'), ('WhatsApp'), ('Site'), ('Instagram'),
  ('Indicação'), ('Google'), ('Visita'), ('Outro')
on conflict (nome) do nothing;

-- Motivos de perda
insert into crm.motivos_perda (nome) values
...
begin
  select id into v_funil from crm.funis where padrao limit 1;
  if v_funil is null then
    insert into crm.funis (nome, ordem, padrao, ativo)
    values ('Comercial Afatec', 0, true, true)
    returning id into v_funil;
  end if;

  insert into crm.etapas (funil_id, nome, ordem, probabilidade, cor, tipo) values
